...
UINT64 HCHead::getCurrentTimestampNs()
{
	...
	// return ts;
	struct timespec time_sty = {0, 0};
	clock_gettime(CLOCK_MONOTONIC, &time_sty);
	return (time_sty.tv_sec * 1000000000 + time_sty.tv_nsec);
}
...
UINT64 HCHead::getCurrentTimestampUs()
{
	...
	struct timespec time_sty = {0, 0};
	clock_gettime(CLOCK_MONOTONIC, &time_sty);
	return (time_sty.tv_sec * 1000000 + time_sty.tv_nsec / 1000);
}
...
UINT64 HCHead::getCurrentTimestampMs()
{
	...
	struct timespec time_sty = {0, 0};
	clock_gettime(CLOCK_MONOTONIC, &time_sty);
	return (time_sty.tv_sec * 1000 + time_sty.tv_nsec / 1000000);
}
