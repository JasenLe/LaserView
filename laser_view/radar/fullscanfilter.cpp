    ...
    std::vector<PointData> &out)
{
    ...
    {
        if(isValidRange(ParaInf, in[i].distance))
        {
            for (int n = 0; n < window_size; n++)
            {
                data_fifo[n] = in[i - 2].distance;
            }
            ...
            {
                if (in[i + j - 2].distance == 0) // 0值不参与取平均
                    continue;

                if (abs(in[i].distance - in[i + j - 2].distance) < dis_threshold) //只有在阈值范围内的点参与求平均
                {
                    cnt++;
                    sum_value += in[i + j - 2].distance;
                }
            }

            if (cnt != 0)
                out[i].distance = sum_value / cnt;
        }
    }
}
    ...
    std::vector<PointData> &out)
{
    ...
    {
        temp_dists_data[i] = in[circle_len - j].distance;
        temp_dists_data[circle_len + i] = in[i].distance;
    }
    ...
    {
        temp_dists_data[i + 2] = in[i].distance;
    }
    ...
    for (i = w_r; i < circle_len + 4 - w_r; i++) ///从第三个点开始，未考虑首尾
    {
        ...
        if (isValidRange(ParaInf, current_data))
        {
            ...
            out[i - w_r].distance = w_d_sum / w_s_sum; //滤波后结果
            w_d_sum = 0.0;
            w_s_sum = 0.0;
        }
    }
    ...
    std::vector<PointData> &out)
{
    ...
    for (int i = w_r; i < count - w_r; i++)
    {
        uint16_t range = in[i].distance; // current lidar distance
        double angle = in[i].angle;           // current lidar angle

        if (range != 0)
        {
            for (int j = -w_r; j < 0; j++)
            {
                diff_1 = abs(in[i + j].distance - in[i].distance);
                diff_2 = abs(in[i - j].distance - in[i].distance);

                LeftPointDistanceCmp = in[i + j].distance;
                RightPointDistanceCmp = in[i - j].distance;
    ...
    std::vector<PointData> &out)
{
    ...
    for (int i = 0; i < circle_len; i++)
    {
        if (in[i].distance < ParaInf.IntesntiyFilterRange)
        {
            if (in[i].confidence <- ParaInf.Weak_Intensity_Th)
            {
                out[i].distance = 0;
                //filter_num_test++;
            }
